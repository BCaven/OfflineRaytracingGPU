...
void VK_Wrap::init()
{
	...
	chk(readback.init(allocator, windowSize.x, windowSize.y, frameImageFormat));
...
void VK_Wrap::clearPrimitives()
{
	vkDeviceWaitIdle(device);   // ensure no in-flight frame is still reading the old buffers

	spheres.clear();
	triangles.clear();
	bvhNodes.clear();
	bvhNodesPacked.clear();
	materials.clear();
	splats.clear();
	transforms.clear();
	shMats.clear();
	...
	kdopColdNodes.clear();
	threadedNodes.clear();
	threadedMatrices.clear();	
}

void VK_Wrap::reloadPrimitives()
{
	loadStructuredBuffer("spheres", spheres);
	loadStructuredBuffer("triangles", triangles);
	loadStructuredBuffer("splats", splats);
...
void VK_Wrap::destroyFrameImage()
{
...
void VK_Wrap::initHistoryImages(bool firstTime)
{
...
void VK_Wrap::updateHistoryDescriptor()
{
...
void VK_Wrap::destroyHistoryImages()
{
...
void VK_Wrap::initShaderCompiler()
{
...
void VK_Wrap::initPipelineLayout()
{
...
void VK_Wrap::initComputePipeline()
{
...
void VK_Wrap::loadStorageImage(std::string bindingName, uint32_t width, uint32_t height, VkFormat format, VkImageUsageFlags extraUsage)
{
	bool firstTime = true;
...
void VK_Wrap::updateImageDescriptors(std::string bindingName)
{
	if (imageBindings.find(bindingName) == imageBindings.end())
	{
...
template<typename T>
void VK_Wrap::loadStructuredBuffer(std::string bindingName, const std::vector<T>& data, size_t elementCount, VkBufferUsageFlags extraUsage)
{
	bool firstTime = true;
...
template <typename T>
void VK_Wrap::loadStructuredBuffer(std::string bindingName, const std::vector<T>& data)
{
	size_t elementCount = std::max(data.size(), (size_t)1);
	loadStructuredBuffer(bindingName, data, elementCount);
}

void VK_Wrap::updateStructuredBufferDescriptors(std::string bindingName)
{
...
void VK_Wrap::resizeRaysBuffer(glm::vec2 windowSize)
{
...
*/
void VK_Wrap::transformAABB(const glm::mat4 M, const glm::vec3& min, const glm::vec3& max, glm::vec3& outMin, glm::vec3& outMax)
{
...
void VK_Wrap::getChildMinMax(const Transform& transform, glm::vec3& min, glm::vec3& max)
{
...
void VK_Wrap::getTransformMinMax(const Transform& transform, glm::vec3& min, glm::vec3& max)
{
...
void VK_Wrap::getSceneBounds(glm::vec3& sceneMin, glm::vec3& sceneMax)
{
...
int VK_Wrap::flattenKDop(int rootIndex)
{
...
int VK_Wrap::build14DOP_parallel(std::vector<KDopLeaf>& leaves, int start, int end, K14Dop& outKDop)
{
...
int VK_Wrap::build14DOP(std::vector<KDopLeaf>& leaves, int start, int end, K14Dop& outKDop)
{
	int count = end - start;
	if (count <= 2)
	{
...
int VK_Wrap::buildSAH(std::vector<LeafItem>& leaves, int start, int end, glm::vec3& outMin, glm::vec3& outMax)
{
...
int VK_Wrap::buildChildBVH(std::vector<LeafItem>& leaves)
{
	if (leaves.size() == 0)
	{
...
ThreadedWrapper VK_Wrap::buildThreadedBVH(PackedRef root, int transformIndex)
{
	// taking an existing tree and converting it to threaded nodes.
	PrimType rootType = unpackType(root);
	int rootIndex = unpackIndex(root);
	...
	{
		Transform t = transforms[rootIndex];
		// all nodes after this need to have this matrix as their miss matrix
		latestTransform = threadedMatrices.size();
		threadedMatrices.push_back(t.invMatrix);
	}
	else // all primitives and empty nodes
	{
		// don't need to add a node
		return ThreadedWrapper{ root, latestTransform };
	}
}
...
PackedRef VK_Wrap::loadTransform(glm::vec3 translation, glm::vec3 rotation, glm::vec3 scale, PackedRef childRef)
{
...
PackedRef VK_Wrap::loadObj(std::string filepath, unsigned int materialIndex)
{
...
PackedRef VK_Wrap::loadSplat(std::string filepath)
{
...
void VK_Wrap::validateBVH(PackedRef rootRef, int depth_to_display)
{
...
void VK_Wrap::validateWideKDop(int root)
{
...
void VK_Wrap::printBVH(int root, int depth_to_display)
{
...
VkBufferMemoryBarrier2 VK_Wrap::bufferBarrier(
	VkBuffer buffer,
...
bool VK_Wrap::draw()
{
	...
	if (readbackFrame == int(frameIndex))
	{
		std::cout << "Writing exr to " << savePath << "\n";
		readback.writeEXR(savePath.c_str(), 1.0f);
		readbackFrame = -1;
		writtenImages += 1;
		if (writtenImages < numImagesPerSequence)
		{
			std::cout << "Processing frame " << writtenImages + 1 << " of " << numImagesPerSequence << "\n";
			// progress to next time and write another image
			saveRequested = true;
			time += time_delta;
			camMoved = true;
		}
	...
	if (finalBounce && saveRequested && readbackFrame < 0)
	{
		if (shaderData.frameCount + 1 >= numFramesPerFile)
		{
			// frameImage is in TRANSFER_SRC_OPTIMAL here (frameImageToTransfer barrier above)
			std::cout << "record frame requested\n";
			readback.record(cb, frameImage);
	...
	quit = KeyInputs::QUIT;
	if (KeyInputs::WRITE_IMAGE && !saveRequested && readbackFrame < 0)
	{
		std::cout << "Save requested to file: " << savePath << "\n";
		saveRequested = true;
	}
	
	
	if (!saveRequested) // lock camera when save requested but can still exit
	{
		...
		if (KeyInputs::NEXT_TIME)
		{
			camMoved = true;
			time += time_delta;
			std::cout << "new time: " << time << "\n";
		}
		if (KeyInputs::PREV_TIME)
		{
			camMoved = true;
			time -= time_delta;
			std::cout << "new time: " << time << "\n";
	...
	if (updateSwapchain) {
		...
		// redo readback
		readback.destroy();
		if (!readback.init(allocator, windowSize.x, windowSize.y, frameImageFormat))
		{
			std::cout << "Hey that was an invalid frameImageFormat!\n";
			quit = true;
		}
		readbackFrame = -1;
		saveRequested = false;
